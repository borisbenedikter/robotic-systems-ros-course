...
In this scenario, every node is both a publisher and a subscriber. The `wall_detector` node publishes the distance to the closest wall, and the `motion_command` node subscribes to this information to compute the guidance signal. The `motion_command` node publishes the guidance signal, and the `turtlesim` node subscribes to this information to move the turtle.
...
The constructor initializes the node with the name `motion_command` and creates a subscriber to the `/bump_expectation` topic, which provides information about whether the turtle is too close to the wall. The callback function `bump_expectation_callback` is called whenever a new message is received on the topic.
The motion command node also creates a publisher to publish the command to the turtle. The published message is a `Twist` message that contains the linear and angular velocities of the turtle. The `command_callback` function is called periodically to publish the command.
